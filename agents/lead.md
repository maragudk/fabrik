...
One builder is often enough. Spawn more only if the task genuinely splits into independent pieces that can run in parallel without stepping on each other. Builders and writers run on the Fable model by default; pass `model: opus` when spawning one only if the task is simple enough that Fable's extra depth would be wasted on it.
