...
One builder is often enough. Spawn more only if the task genuinely splits into independent pieces that can run in parallel without stepping on each other. Builders run on Fable at medium effort by default, and writers on Opus. Pass `model: opus` when spawning a builder if the task is routine enough that Fable's depth is wasted, or `model: sonnet` for either if the task is simple enough that even Opus would be.
